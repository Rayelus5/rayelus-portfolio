...
const tokensBar = document.querySelector("#tokens-erc");
//
//Constants for body
...
function startGame() {
	...
	//Tamagotchi's name
	var tamagotchiName = prompt("Please, enter a name of your kouchi:", "");
	document.querySelector("#tokenBoost").innerHTML = tokenBoost;
	document.querySelector("#name").innerHTML = tamagotchiName;
	if (tamagotchiName == null || tamagotchiName.replace(/\s/g, '') == "") {
		tamagotchiName = "Kouchi";
		document.querySelector("#name").innerHTML = tamagotchiName;
		document.querySelector("#tokenBoost").innerHTML = tokenBoost;
	}
	...
	function core() {
		...
		//Scores
		score = score + 1;
        tokens = (score * tokenBoost) / (10*tokenDown);
		scoreBar.innerHTML = score;
        tokensBar.innerHTML = tokens;
        ...
        if ((tokens >= 200)){
            ...
            tokens = 200;
			clearInterval(coreUpdate);
			alert('Maximum ERC tokens recolected today (200 ERC) \n ╭(×_×)╮');
		}
